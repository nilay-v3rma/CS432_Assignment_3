...
* **`scripts/exp1_concurrent_updates.py`**: Tests isolation and consistency by having multiple simulated users rapidly attempt to update the exact same resource.
...
* **`scripts/exp3_failure_simulation.py`**: Submits transactions flagged to fail midway through execution. Designed to verify that the database correctly rolls back changes (Atomicity).
* **`scripts/exp4_stress_test.py`**: Floods the API with a heavy mix of read and write requests to measure system stability, throughput, and latency under high concurrency.

## 3. How to Run Each Experiment
Ensure you have Python and Locust installed (`pip install locust`), and your backend is running. Run these from the root of the workspace.
